...
**/
	...
	try{
		...
		sql = "update staff_list set userPw=sha(?),userName=?,userBirthMonth=?,userBirthDay=?,userPhone1=?,userPhone2=?,userPhone3=? where userId =?";
		psmt = conn.prepareStatement(sql);
		psmt.setString(1, joinPw1);
		psmt.setString(2, joinName);
		psmt.setString(3, joinBirthMonth);
		psmt.setString(4, joinBirthDay);
		psmt.setString(5, joinPhone1);
		psmt.setString(6, joinPhone2);
		psmt.setString(7, joinPhone3);
		psmt.setString(8, joinId);
		...
		out.print("<script>alert('수정이 완료되었습니다.');window.location='http://localhost:8080/CafeDeHorang/myInfo.jsp'</script>");
	}
%> 